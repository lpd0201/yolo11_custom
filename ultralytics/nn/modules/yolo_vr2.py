...
#             Conv(c1, c2, k=1),
#             Conv(c2, c2, k=(5, 1), p=(2, 0)),
#             Conv(c2, c2, k=(1, 5), p=(0, 2))
#         )
#         self.x3_2 = nn.Sequential(
#             Conv(c2, c2, k=(7, 1), p=(3, 0)),
#             Conv(c2, c2, k=(1, 7), p=(0, 3))
#         )
...
#         # out_gap = self.x4(self.mp(x)) 
#         # out_gap = self.x4(self.mp(x))
#         # out4 = out_gap.expand(-1, -1, x.shape[2], x.shape[3])
...
#         return self.silu(self.batchNorm(res + high_features + low_features))
...
# #             nn.SiLU(inplace=True)
# #         )
# #         self.max1 = nn.MaxPool2d(kernel_size=k, stride=1, padding=k//2)
# #         self.max2 = nn.MaxPool2d(kernel_size=k, stride=1, padding=k//2)
# #         c_strip = c1 // 4
# #         self.branch_strip = nn.Sequential(
# #             nn.Conv2d(c1, c_strip, kernel_size=(1, k), padding=(0, strip), groups=c_strip, bias=False),
# #             nn.BatchNorm2d(c_strip),
# #             nn.SiLU(inplace=True),
# #             nn.Conv2d(c_strip, c_strip, kernel_size=(k, 1), padding=(strip, 0), groups=c_strip, bias=False),
# #             nn.BatchNorm2d(c_),
# #             nn.SiLU(inplace=True)
# #         )
...
# #             nn.Conv2d(c2 + c_strip, c2, kernel_size=1, stride=1, bias=False),
# #             nn.BatchNorm2d(c2),
# #             nn.SiLU(inplace=True)
# #         )
...
# #         m2 = self.max1(m1)
# #         m3 = self.max1(m2)
# #         out_sppf = self.cv2(torch.cat([y, m1, m2, m3], dim=1))
        
# #         # --- Thực thi Nhánh 2 (Strip Expert) ---
# #         out_strip = self.branch_strip(x)
        
# #         # --- DUNG HỢP TỔNG THỂ (Concat + Fusion 1x1) ---
# #         # Nối dọc theo chiều Channel và trộn đặc trưng xuất ra đúng c2 kênh
# #         return self.fushion(torch.cat([out_sppf, out_strip], dim=1))
# import torch
# import torch.nn as nn
...
# ==========================================
# 1. Khối FasterNet (Hộp màu Vàng trong sơ đồ)
# ==========================================
class FasterNetBlock(nn.Module):
    def __init__(self, c):
        super().__init__()
        # Theo sơ đồ: 3 lớp PConv liên tiếp, lớp giữa có BN & GELU
    ...
    def forward(self, x):
        # Đường residual nét liền đi ngang + đường đi qua các PConv
        res = x
        x = self.pconv1(x)
        x = self.pconv2(x)
        x = self.bn_gelu(x)
        x = self.pconv3(x)
        return x + res  # Phép cộng (dấu + trong vòng tròn)

# ==========================================
# 2. Khối RexHazy (Hộp màu Xanh dương) - Code của bạn đã chuẩn
# ==========================================
...
class RFC3k2(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5, *args, **kwargs):
        ...
        # 2. Hai nhánh song song (Thay vì dùng ModuleList và for loop)
