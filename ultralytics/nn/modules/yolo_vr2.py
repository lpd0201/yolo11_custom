...
class EE_Block(nn.Module):
    ...
    def forward(self, x):
        high_freq = x - self.avg_pool(x) 
        edge_feat = self.dw_conv(high_freq) 
...
class IndirectlyPathContextGuide(nn.Module):
    def __init__(self, c_list, r=16):
        ...
        self.spatial_guide = nn.Sequential(
            nn.Conv2d(p_i, p_i, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(p_i)
        )
        nn.init.constant_(self.spatial_guide[0].weight, 0)
        nn.init.constant_(self.spatial_guide[1].weight, 1)
        nn.init.constant_(self.spatial_guide[1].bias, 0)
        self.od_lska = HOD_LSKA(dim=p_i1, k_size=11)
        self.ee_block = EE_Block(p_i)
    ...
    def forward(self, x):
        ...
        if mid_up.shape[2:] != p_i.shape[2:]:
            mid_up = F.interpolate(mid_up, size=p_i.shape[2:], mode='bilinear', align_corners=False)
        f_out_shallow = mid_up + p_i
        spatial_mask = torch.sigmoid(self.spatial_guide(f_out_shallow)) 
        high_freq = p_i - self.ee_block.avg_pool(p_i)
        edge_raw = self.ee_block.dw_conv(high_freq)
        edge_feat = edge_raw * self.ee_block.gate(p_i)
        
        # Cộng chính xác: Đặc trưng gốc + (Đặc trưng Cạnh x Mặt nạ Không gian)
        final_features = p_i + (edge_feat * spatial_mask)
